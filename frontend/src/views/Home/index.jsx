...
const Home = () => {
  ...
  You can use the menu area on the right hand side of this page to navigate between different areas of the application.
  ...
  On each of these pages there is a link at the top which will show you an HLS Player representing you chosen item.

  - **MediaLive Channels** shows a list of AWS Elemental MediaLive Channels. You can choose to start (or stop) and Ingest workflow and/or start/stop the Channel.
  ...
  In order to ingest content from a MediaLive Channel into TAMS it needs both an Ingest Workflow running and the Channel to be running.
  ...
  - A Media to be ingested. This file should be placed in the same sub-folder as the job \`mediaconvert.json\` file (this controls which job settings will be used for the ingestion).
  ...
  ***IMPORTANT: The underscore character should not be used in the naming of the folder or the media file name. Doing so will prevent the ingest from running.***
  ...
  | Tag Key                  | Type   | Notes |
  | ------------------------ | ------ | ----- |
  | ***use_start_epoch***    | _bool_ | If set with a value of **true** then the timerange value for ingested segments will be based upon the EPOCH. If it has a different value or is missing the timerange will be zero based |
  ...
  | ***hls_segments***       | _number_ | Used to limit the number of segments presented in the HLS manifest. Default to **150** if tag not set. Use the value **inf** to list all segments. However,listing all segments may result in the generation of the HLS mnaifest timing out. |
