...
ffmpeg_bucket = os.environ.get("FFMPEG_BUCKET", "")
...
def lambda_handler(event, context):
    ...
    # File came from ffmpeg output
    if bucket == ffmpeg_bucket and key.startswith(ffmpeg_prefix):
        flow_id, timerange = key[len("ffmpeg/") :].split("_", 1)
        media_object = upload_file(bucket, key, flow_id)
        if media_object["put_url"]["content-type"].split("/")[0] == "image":

            timerange = f'{timerange.split("_")[0]}]'
        post_segment(flow_id, media_object["object_id"], timerange)
        s3.Object(bucket, key).delete()
    # File came from MediaLive or MediaConvert output
    else:
